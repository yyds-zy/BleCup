package com.bluetooth.cup.view;
...
import com.bluetooth.cup.BuildConfig;
...
public class WaveProgress extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mValueColor = typedArray.getColor(R.styleable.WaveProgress_valueColor, Color.GRAY);
        ...
        mCircleColor = typedArray.getColor(R.styleable.WaveProgress_circleColor, Color.BLUE);
        mBgCircleColor = typedArray.getColor(R.styleable.WaveProgress_bgCircleColor, Color.parseColor("#FFE6E8FA"));
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (SharePreferenceUtil.getBoolean(SharePreferenceUtil.DEVICE_CONNECT_STATE,false)) {
...
//        if (!SharePreferenceUtil.getBoolean("cup_connect_state",false)) {
//            mPercent = 0;
//        }
        canvas.save();
...
//        }
        if (mPrePercent == 0.0f || Math.abs(mPercent - mPrePercent) >= 0.01f) {
            mPercentValue = String.format("剩余电量：%.0f%%", mPercent * 100);
            mPrePercent = mPercent;
        }
        canvas.drawText(mPercentValue, mCenterPoint.x, y + 250, mPercentPaint);
